...
def generar_html_informe(
    ...
    logo_base64 = img_to_base64("assets/logo_elche.png")
...
with tab1:
    try:
        ...
        if st.button("🔍 Clasificar"):
            ...
            df_topk = pd.DataFrame(result["top_k"])
            if not df_topk.empty:
                # Renombrado más amigable
                ...
                st.dataframe(df_topk, use_container_width=True, hide_index=True)
            else:
                st.info("No hay patrones TOP-K disponibles para mostrar.")
            ...
            with col1:
                ...
                if exp.get("verbos_comunes"):
                    for v in exp["verbos_comunes"]:
                        st.write(f"• {v}")
                else:
                    st.write("—")

            with col2:
                st.markdown("**Solo en la selección**")
                if exp.get("solo_en_usuario"):
                    for v in exp["solo_en_usuario"]:
                        st.write(f"• {v}")
                else:
                    st.write("—")

            with col3:
                st.markdown("**Solo en el patrón de referencia**")
                if exp.get("solo_en_top1"):
                    for v in exp["solo_en_top1"]:
                        st.write(f"• {v}")
                else:
                    st.write("—")
